...
class AsymmetricView extends StatefulWidget {
  ...
  const AsymmetricView({Key key, this.products, this.isFetching, this.isEnd, this.onLoadMore, this.width});
...
class _AsymmetricViewState extends State<AsymmetricView> {
  ...
  List<Container> _buildColumns(BuildContext context) {
    ...
    var productsList = List.generate(_listItemCount(widget.products.length), (int index) {
      double minWidth = math.min(widget.width, 400);
      double width = .59 * minWidth;
      Widget column;
      if (index % 2 == 0) {
        /// Even cases
        ...
        column = TwoProductCardColumn(
            ...
            top: widget.products.length - 1 >= bottom + 1 ? widget.products[bottom + 1] : null);
        width += 32.0;
      } else {
        /// Odd cases
