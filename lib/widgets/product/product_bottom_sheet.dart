...
class ExpandingBottomSheet extends StatefulWidget {
  const ExpandingBottomSheet(
      {Key key, this.hideController, this.onInitController})
...
//  }
  ...
  EdgeInsetsDirectional _cartPaddingFor(int numProducts) {
    if (numProducts == 0) {
      return EdgeInsetsDirectional.only(start: 10.0, end: 8.0);
    } else {
      return EdgeInsetsDirectional.only(start: 16.0, end: 8.0);
    }
  }
        ...
        child: Column(children: <Widget>[
          Row(children: <Widget>[
            ...
            Container(
              ...
              width: numProducts < 1
                  ? _width - 50.0
                  : numProducts > 3 ? _width - 110.0 : _width - 75.0,
              height: _kCartHeight,
...
class _ProductThumbnailRowState extends State<ProductThumbnailRow> {
  ...
  Product _productWithId(String key) {
    ...
    final Product product = Provider.of<CartModel>(context, listen: false)
...
//    assert(product != null);
  ...
  void _updateLists() {
    ...
    _internalList = Provider.of<CartModel>(context, listen: false)
        ...
        .keys
        .toList();
    Set<String> internalSet = Set<String>.from(_internalList);
    Set<String> listSet = Set<String>.from(_list.list);

    Set<String> difference = internalSet.difference(listSet);
...
class ExtraProductsNumber extends StatelessWidget {
  ...
  int _calculateOverflow(CartModel model) {
    Map<String, int> productMap = model.productsInCart;
    ...
    List<String> products = productMap.keys.toList();
    int overflow = 0;
