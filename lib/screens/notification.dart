...
//        print(err);
  ...
  String getTime(String time) {
    ...
    if (now.difference(date).inHours > 0) {
      return '${now.difference(date).inHours} hours ago';
    }
    if (now.difference(date).inMinutes > 0) {
      return '${now.difference(date).inMinutes} minutes ago';
    }
    return '${now.difference(date).inSeconds} seconds ago';
  }
...
//      print(err);
  ...
  Future<void> _showAlert(BuildContext context, Map<String, dynamic> data, int index) async {
    FStoreNotification a = FStoreNotification.fromLocalStorage(data);
    await a.updateSeen(index);
    try {
      final ready = await storage.ready;
      if (ready) {
        var list = storage.getItem('notifications');
