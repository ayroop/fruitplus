...
class _BlogsPageState extends State<BlogsPage> with SingleTickerProviderStateMixin {
  int newCategoryId = -1;
  double minPrice;
  double maxPrice;
  String orderBy;
  ...
  Future<void> onRefresh() async {
    if (widget.config == null) {
      ...
      try {
        var newProducts = await Services()
            .fetchProductsLayout(config: widget.config, lang: Provider.of<AppModel>(context, listen: false).locale);
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableProvider.value(
      ...
      child: Consumer<BlogNewsModel>(builder: (context, value, child) {
        return backdrop(blogs: value.blogList, isFetching: value.isFetching, errMsg: value.errMsg, isEnd: value.isEnd);
      }),
    );
  }
}
